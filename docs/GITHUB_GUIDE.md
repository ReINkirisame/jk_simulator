...
当前发行版为0.6.0。下面0.2～0.4的操作示例保留作历史参考；更新本版时使用新的独立分支，复制源码包中的项目内容，执行 `npm test` 后再审阅合并。当前版本已支持三年基础流程、生活习惯、条件传闻、毕业档案、多特质成长和角色合成，规则以 README 与 RELEASE_0.6.0.md 为准。
...
4. 不要动仓库里的隐藏 `.git` 文件夹。
5. 双击 `index.html` 试玩；如候选池没有目标成长特质，可使用页面底部调试面板准备合成线。可以运行 Node.js 时再执行 `npm test`。
...
4. 重点试玩四件事：普通癫佬大失败、双来源隐藏合成、精力压力透支、月考与期末考试。
